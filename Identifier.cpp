...
Parser::Parser<Identifier> identifier_qualified() {
	...
	return transform(
	    ...
	    [](std::tuple<std::optional<Token>, std::vector<Token>> const &data) -> Identifier {
		    ...
		    // if we just received a regular identifier with no ::, it's just a regular identifier
		    ...
		    return Identifier{.kind = Identifier::Kind::Qualified, .value = qualified_path_object};
	    });
}
